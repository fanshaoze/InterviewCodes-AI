...
class BasicRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for t in range(x.size(1)):
            ...
            hidden_state = self.relu(
                self.input_to_hidden(input_t) + self.hidden_to_hidden(hidden_state))  # Update hidden state
...
# Define the Linear-based model
class BasicGRU(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, output_size):
        super(BasicGRU, self).__init__()
        self.hidden_size = hidden_size
        self.embedding = nn.Embedding(vocab_size, embed_size)
        self.input_to_hidden = nn.Linear(embed_size + hidden_size, hidden_size)
        self.hidden_to_hidden = nn.Linear(hidden_size, hidden_size)
        self.hidden_to_output = nn.Linear(hidden_size, output_size)
        self.candidate_to_hidden = nn.Linear(hidden_size, hidden_size)
    def forward(self, x):
        h = torch.zeros(x.size(0), self.hidden_size)
        h = h.to(device)
        for t in range(x.size(1)):
            input_t = self.embedding(x[:, t])
            combined = torch.cat((input_t, h), dim=1)
            r = torch.sigmoid(self.input_to_hidden(combined))
            z = torch.sigmoid(self.hidden_to_hidden(h))
            n = torch.tanh(self.candidate_to_hidden(input_t) + r * h)
            h = (1 - z) * n + z * h
        output = self.hidden_to_output(h)
        return output


# Define the GRU-based model with nn.GRU
class GRUModel_Torch(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        super(GRUModel_Torch, self).__init__()
        self.embedding = nn.Embedding(input_size, hidden_size)
    ...
    def forward(self, x):
        ...
        output = self.fc(gru_out[:, -1, :])  # Use the last hidden state
        return output
...
def train_with_GRU():
    model = BasicGRU(vocab_size, embedding_dim, hidden_size, output_size)
    total_params = count_parameters(model)
...
def train_with_GRUModel_Torch():
    model = GRUModel_Torch(input_size, hidden_size, output_size)
    total_params = count_parameters(model)
    print(f"Torch GRU Trainable Params: {total_params}")
    criterion = nn.CrossEntropyLoss()  # For classification
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    num_epochs = 100
    loss_list = train_batch(X, model, num_epochs, criterion, optimizer)
...
_, lossGRU = train_with_GRU()
_, lossGRU_torch = train_with_GRUModel_Torch()
print(lossRNN, '\n', lossRNN_torch, '\n',lossGRU, '\n',lossGRU_torch)
plt.plot(lossRNN)
...
plt.plot(lossGRU)
plt.plot(lossGRU_torch)
plt.legend(['basic RNN', 'nn.RNN', 'basic LSTM', 'nn.LSTM', 'basic GRU', 'nn.GRU'])
